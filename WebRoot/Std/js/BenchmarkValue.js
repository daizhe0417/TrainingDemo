var jqGridConf = {
    ...
    colModel: [{
        ...
        index: 'benchmarkId',
        hidden: true,
        selectDisplayFieldName: 'benchmarkName',
        queryType: 'equal'
    }, {
        name: 'benchmarkName',
        index: 'benchmarkName',
...
function setBenchmark_Sel_Callback(item) {
    var sel1 = $("#query_benchmarkId");
    var sel2 = $("#benchmarkId");
    if (item.datas != null) {
        if (sel1.length > 0) {
            sel1.empty();
            sel1.html("<option value=''>--请选择业绩基准--</option>");
        }
        if (sel2.length > 0) {
            sel2.empty();
            sel2.html("<option value=''>--请选择业绩基准--</option>");
        }
        var list = item.datas;
...
function onChartBtn() {
    var benchmarkId = $("#query_benchmarkId").val();
