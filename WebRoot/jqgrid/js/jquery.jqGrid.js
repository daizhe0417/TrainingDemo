	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-first png ui-icon-seek-first-pngs'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-prev png  ui-icon-seek-prev-pngs'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-next png  ui-icon-seek-next-pngs'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-end png  ui-icon-seek-end-pngs'></span></td>";
...
//					alert('vl'+vl);
					...
					var callDwrRmoter=function(data,elem,vl,options){
//						alert('elem in '+elem)
//						alert('vl in '+vl);
//						alert('option in '+options)
						var ovm = [], 
...
//							alert('a:'+a);
							...
							for(var i=0;i<a.length;i++){
								optionStr+="<option value='"+a[i].value+"'>"+a[i].name+"</option>";
							}
							$(elem).append(optionStr);
							setAttributes(elem, options);
					...
					eval(options.dataUrl+"(function(item){callDwrRmoter(item,elem,vl,options);})");
//					$.ajax($.extend({
//						url: options.dataUrl,
//						type : "GET",
//						dataType: "html",
...
$.jgrid.extend({
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function postIt() {
				to_save();
				//var formMap=DWRUtil.getValues("FrmGrid_jqGridList");
				//eval(actionname + ".save(formMap,toQuery)");
...
//									}
//								}
						//   要留下收尾的工作，目前只知道有不再显示删除窗口的代码，其他的不知道干什么的
								rp_ge[$t.p.id].processing=false;
